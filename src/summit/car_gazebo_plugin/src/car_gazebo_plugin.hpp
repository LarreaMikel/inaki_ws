...
class CarGazeboPlugin : public gazebo::ModelPlugin {
 ...
 private:
  ...
  double steer = 0;
  double velocity = 0;
  const double wheelbase_length = 0.372;
  const double front_wheelbase_width = 0.21;
  const double rear_wheelbase_width = 0.33;
  const double wheel_diameter = 0.16;
  const double max_speed = 3;
  const double max_turn_angle = M_PI/4.0;
  AckermannModel car_model = {0.21, 0.33};
/*
  double steer = 0;
  double velocity = 0;
...
*/
  ...
  double steer = 0;
  double velocity = 0;
  const double wheelbase_length = 0.458;
  const double front_wheelbase_width = 0.471;
  const double rear_wheelbase_width = 0.471;
  const double wheel_diameter = .235;
  const double max_speed = 3;
  const double max_turn_angle = M_PI/4.0;
  AckermannModel car_model = {0.471, 0.471};
  https://www.roscomponents.com/en/mobile-robots/17-summit-xl.html#/encoders-no/gps-no/front_connection_for_docking-no/top_load_plate_65_kg_payload_only_gps_sensor-no/imu-imu_vn_100t_cr_vectornav/cpu-jnf797_q370_intel_9th_gen_core_i7_ssd_250gb_ram_32gb/omni_wheels-rubber_wheels/docking_station-no/summit_xl_rgbd_zone-no/summit_xl_zone_3-no/chasis_ip-standard/summit_xl_zone_2-no/router_4g-no
  */
